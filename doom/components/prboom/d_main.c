...
bool D_AddFile(const char *file)
{
  ...
  // esp_littlefs (this port's WAD storage) doesn't implement access_p, so
  // access() always fails on littlefs paths even for files that genuinely
  // exist - use stat() instead, which esp_littlefs does implement.
  struct stat st;
